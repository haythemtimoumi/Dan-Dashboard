...
export async function PUT(request: NextRequest, { params }: { params: { ticker: string } }) {
  try {
    ...
    const response = await fetch(`http://localhost:8000/api/stocks/ticker/${ticker}/ticker-view`, {
      method: 'PUT',
      headers: {
    ...
    if (!response.ok) {
      return NextResponse.json({ error: 'Failed to update ticker view' }, { status: response.status });
    }
