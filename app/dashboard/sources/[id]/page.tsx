...
export default async function SourceStockDetailPage({ params }: { params: { id: string } }) {
  const id = params.id;
  ...
  try {
    ...
    if (!stock) {
      console.log(`Source stock with ID ${id} not found, showing user-friendly message`);
      ...
      <main>
        <div className="flex items-center justify-between mb-8">
          <h1 className={`${lusitana.className} text-2xl md:text-3xl font-bold text-blue-900`}>
            {stock.source} Stock Details: {stock.ticker}
          </h1>
          <Link
            href="/dashboard/sources"
