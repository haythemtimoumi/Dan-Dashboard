...
export default function GoldStockAnalysisDetailPage({ params }: { params: { ticker: string } }) {
  ...
  const fetchData = async (date: string) => {
    try {
      ...
      const response = await fetch(`/api/stocks/by-ticker-date?ticker=${tickerSymbol}&date=${date}`);
      
      if (!response.ok) {
        setError(`No data found for ${tickerSymbol}`);
        setData(null);
        return;
      }
  ...
  if (error || !data || data.gurus.length === 0) {
    return (
      <div className="text-center py-8">
        ...
        <Link href="/dashboard/gold-stocks" className="text-blue-600 hover:underline mt-2 inline-block">
          Back to Gold Stocks
        </Link>
      </div>
    );
        ...
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center gap-4">
            ...
            <div className="flex items-start gap-6">
              <div>
                <h1 className="text-3xl font-bold">{tickerSymbol} ({language === 'fr' ? 'Cible' : 'Target'})</h1>
      ...
      <div className="flex flex-col lg:flex-row gap-4 h-[500px]">
        ...
        <div className="w-full lg:w-[70%] bg-white/70 dark:bg-gray-800/70 backdrop-blur-lg rounded-lg shadow-lg p-4 border border-white/20">
          ...
          <div className="h-[450px]">
            <TradingViewWidget ticker={tickerSymbol} height="450px" />
          </div>
        </div>
