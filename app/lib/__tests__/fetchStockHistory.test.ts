...
describe('fetchStockHistory', () => {
  beforeEach(() => {
    ...
    process.env.NEXT_PUBLIC_API_URL = 'http://locahost:3000/api';
  });
  ...
  it('should fetch stock history with the correct URL', async () => {
    ...
    expect(fetch).toHaveBeenCalledWith('http://locahost:3000/api/stocks/1/history');
  ...
  it('should include from and to parameters when provided', async () => {
    ...
    expect(fetch).toHaveBeenCalledWith(
      `http://locahost:3000/api/stocks/1/history?from=${fromDate}&to=${toDate}`
    );
  });
  ...
  it('should return empty array when stock history is not found (404)', async () => {
    ...
    // Verify the URL used in the fetch call
    expect(fetch).toHaveBeenCalledWith('http://locahost:3000/api/stocks/999/history');
